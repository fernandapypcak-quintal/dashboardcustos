...
export default function CustoVariavel() {
  ...
  const porCatLoja = useMemo(()=>{
    const map={}
    custosVariaveisFiltrados.forEach(({categoria,loja,orcado,realizado})=>{
  ...
  const baseItens = lojaMode==='geral' ? porCatLoja : custosVariaveisFiltrados.map(c=>({...c,variacao:c.orcado>0?((c.realizado-c.orcado)/c.orcado)*100:0}))
  ...
  return (
    <div style={{ background:'#fff', minHeight:'100vh' }}>
      ...
      <div style={{ padding:'24px 28px', display:'flex', flexDirection:'column', gap:20 }}>
        ...
        {tabelaHistorica.length>0 && (
          <div style={{ border:'1px solid #F0F0F0', borderRadius:12, overflow:'hidden' }}>
            <div style={{ padding:'14px 20px', borderBottom:'1px solid #F7F7F7' }}>
              ...
              <div style={{ fontSize:12, color:'#999', marginTop:2 }}>Variação em destaque</div>
            </div>
        ...
        {lojaFiltro==='Todas' && (
          <div style={{ border:'1px solid #F0F0F0', borderRadius:12, overflow:'hidden' }}>
            <div style={{ padding:'14px 20px', borderBottom:'1px solid #F7F7F7', fontSize:13, fontWeight:600 }}>Custo Variável por Loja — mês atual</div>
            <table style={{ width:'100%', borderCollapse:'collapse' }}>
        ...
        <div style={{ border:'1px solid #F0F0F0', borderRadius:12, padding:'20px 24px 16px' }}>
          <div style={{ fontSize:13, fontWeight:600, marginBottom:18 }}>Orçado vs Realizado por Categoria — mês atual</div>
        ...
        <div style={{ border:'1px solid #F0F0F0', borderRadius:12, overflow:'hidden' }}>
          <div style={{ padding:'14px 20px', borderBottom:'1px solid #F7F7F7', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:10 }}>
            <div>
              <div style={{ fontSize:13, fontWeight:600 }}>Itens de Custo Variável</div>
              <div style={{ fontSize:12, color:'#999', marginTop:2 }}>{lojaMode==='geral'?'Totais por categoria consolidados':'Detalhado por subcategoria e loja'}</div>
            </div>

...
const CORES_CAT = { 'CMV': '#0D0D0D', 'Comissões': '#97A624', 'Embalagens': '#D9B504', 'Mão de obra variável': '#8C1414' }
...
const TD = ({ children, mono, color }) => (
  <td style={{ padding: '10px 14px', fontSize: 13, color: color || '#0D0D0D', borderBottom: '1px solid #F0F0E8', fontFamily: mono ? "'DM Mono', monospace" : undefined }}>
    {children}
  </td>
)
function VarBadge({ pct }) {
  ...
  return <span style={{ display: 'inline-block', padding: '2px 8px', borderRadius: 20, fontSize: 11, fontWeight: 700, fontFamily: "'DM Mono', monospace", background: bg, color }}>{fmtPct(pct)}</span>
}
const S = (ativo) => ({
...
export default function CustoVariavel() {
  const { custosVariaveisFiltrados, historicoCatVariavelFiltrado, historicoVariavel, lojaFiltro } = useFinanceiro()
  ...
  const porItem = useMemo(() => custosVariaveisFiltrados.map((c) => ({ ...c, variacao: c.orcado > 0 ? ((c.realizado - c.orcado) / c.orcado) * 100 : 0 })), [custosVariaveisFiltrados])
  ...
  // Pivot histórico por loja para gráfico de linhas
  const lojas = useMemo(() => Array.from(new Set(historicoVariavel.map((h) => h.loja))), [historicoVariavel])
  const mesesHist = useMemo(() => Array.from(new Set(historicoVariavel.map((h) => h.mes))), [historicoVariavel])
  const dadosLinha = useMemo(() => mesesHist.map((mes) => {
    const row = { mes }
    lojas.forEach((loja) => {
    ...
    return row
  }), [mesesHist, lojas, historicoVariavel, metricHist])
      ...
      <div style={{ padding: '20px 28px', display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div style={{ background: '#fff', border: '1px solid #E8E8E2', borderRadius: 8, overflow: 'hidden' }}>
              <div style={{ padding: '12px 16px', borderBottom: '1px solid #E8E8E2', display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: 10.5, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#8A8A7A' }}>Maiores Altas — mês a mês</span>
              </div>
              {ranking.maioresAltas.map((r, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 16px', borderBottom: '1px solid #F0F0E8' }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ width: 8, height: 8, borderRadius: '50%', background: CORES_CAT[r.categoria] || '#ccc' }} />
            ...
            <div style={{ background: '#fff', border: '1px solid #E8E8E2', borderRadius: 8, overflow: 'hidden' }}>
              <div style={{ padding: '12px 16px', borderBottom: '1px solid #E8E8E2', display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: 10.5, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#8A8A7A' }}>Maiores Quedas — mês a mês</span>
              </div>
          ...
          <div style={{ background: '#fff', border: '1px solid #E8E8E2', borderRadius: 8, padding: '16px 20px' }}>
            ...
            <ResponsiveContainer width="100%" height={240}>
              <LineChart data={dadosGrafico} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                ...
                {categorias.map((cat) => (
                  <Line key={cat} type="monotone" dataKey={cat} stroke={CORES_CAT[cat] || '#0D0D0D'} strokeWidth={2} dot={{ r: 3 }} activeDot={{ r: 5 }} />
                ))}
              </LineChart>
            </ResponsiveContainer>
          </div>
        ...
        <div style={{ background: '#fff', border: '1px solid #E8E8E2', borderRadius: 8, padding: '16px 20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            <span style={{ fontSize: 10.5, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#8A8A7A' }}>Evolução por Loja</span>
        ...
        {lojaFiltro === 'Todas' && (
          <div style={{ background: '#fff', border: '1px solid #E8E8E2', borderRadius: 8, overflow: 'hidden' }}>
            <div style={{ padding: '12px 18px', borderBottom: '1px solid #E8E8E2' }}>
              <span style={{ fontSize: 10.5, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#8A8A7A' }}>Custo Variável por Loja — mês atual</span>
            </div>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        ...
        <div style={{ background: '#fff', border: '1px solid #E8E8E2', borderRadius: 8, padding: '16px 20px' }}>
          <div style={{ fontSize: 10.5, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#8A8A7A', marginBottom: 16 }}>Orçado vs Realizado — mês atual</div>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={porCategoria} margin={{ top: 4, right: 4, left: 0, bottom: 0 }} barCategoryGap="30%">
              ...
              <Bar dataKey="realizado" name="realizado" radius={[3,3,0,0]}>
                {porCategoria.map((e) => <Cell key={e.categoria} fill={CORES_CAT[e.categoria] || '#97A624'} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
